...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (containerCards) {
    containerCards.innerHTML = alunos.map(aluno => `
      <div class="card">
        <div class="card-image-container">
          <img src="${aluno.imagem}" alt="Aluno ${aluno.nome}" class="card-image" loading="lazy">
        </div>
        <div class="card-content">
          <h3 class="card-title">${aluno.nome}</h3>
          <p class="card-text">${aluno.descricao}</p>
          <button class="saiba-mais btn-detalhes" data-id="${aluno.id}">Saiba +</button>
        </div>
      </div>
    `).join('');
  }
  ...
  document.addEventListener('click', (event) => {
    ...
    if (event.target.classList.contains('btn-detalhes')) {
      ...
      if (dadosEntrevista && secaoDetalhes) {
        ...
        const temVideo = dadosEntrevista.video !== undefined && dadosEntrevista.video !== null && dadosEntrevista.video !== "";
        ...
        const htmlDoVideo = temVideo ? `
          <div class="video-container" style="margin-top: 20px; text-align: center;">
            <video src="${dadosEntrevista.video}" controls style="max-width: 100%; max-height: 450px; border-radius: 6px; box-shadow: 0 2px 8px rgba(0,0,0,0.15);"></video>
          </div>
        ` : '';
        ...
        secaoDetalhes.innerHTML = `
            <div class="info-container">
              ...
              <div class="info-content">
                <div class="info-bio">
                  <img src="${dadosEntrevista.imagemCapa}" alt="${dadosEntrevista.imagemAlt ?? 'Foto do Aluno'}" class="info-image">
                  ${htmlDosParagrafos}
                  ${htmlDoVideo}
                </div>
      ...
      if (!modal) {
        ...
        modal.innerHTML = `
          <div id="pdf-container" style="width: 85%; height: 90%; background: #ffffff; border-radius: 8px; display: flex; flex-direction: column; overflow: hidden; box-shadow: 0 10px 25px rgba(0,0,0,0.5);">
            ...
            <!-- Barra superior isolada para o botão de fechar -->
